...
export default function Game({client}) {
  ...
  useEffect(()=>{
    client.send(JSON.stringify({type: 'my_mana',adress: enemyId,myMana: myMana}));
  },[myMana])
      ...
      const dataFromServer = JSON.parse(message.data);
          if(dataFromServer.type==='data'){
            ...
            if(!whoIAm && !whoIsEnemy && allDataRequired.current){
              ...
              const guest = data[data.findIndex(obj=>obj.room === params.roomNumber)].players.guest.id == params.uniqueId;
                if(guest){
                  setWhoIAm('guest')
                  setWhoIsEnemy('host');
                  const myUser = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['guest'].username;
                  const enemyUser = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['host'].username;
                  setMyUserName(myUser);
                  setEnemyUserName(enemyUser);
                  const enemyUniqueId = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['host'].id;
                  setEnemyId(enemyUniqueId);
                }else {
                  setWhoIAm('host');
                  setWhoIsEnemy('guest')
                  setMySide('light');
                  const myUser = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['host'].username;
                  const enemyUser = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['guest'].username;
                  setMyUserName(myUser);
                  setEnemyUserName(enemyUser);
                  const enemyUniqueId = data[data.findIndex(obj=>obj.room === params.roomNumber)].players['guest'].id;
                  setEnemyId(enemyUniqueId);
                }
            }
            }
  ...
  useEffect(()=>{
    if(whoIAm === 'host'){
      ...
      if(whoIsFirst == 1){
        ...
        client.send(JSON.stringify({type: 'my_turn',adress: enemyId,turn:{turn:true,round:1}}));
      }else {
        whoIsJedi.current = false;
        client.send(JSON.stringify({type: 'your_turn',adress: enemyId,turn:{turn:true,round:1}}));
        setEnemyTurn({
          turn:true,
          round : 1,
        })
  ...
  return (
    ...
    <Conclusion style={{display : `${gameFinished ? 'inline-flex' : 'none'}`}} myHp={myHp} enemyHp={enemyHp}/>
    ...
    <canvas id="canvas"></canvas>
      ...
      <div className="trash-container" style={{display : `${reborn.active ? 'block' : 'none'}`}}>
      <button onClick={(event)=>{event.nativeEvent.path[1].style.display = 'none'}}>Exit</button>
        {
          trashCopy.map((itm,index)=>{
            return (
              <img onClick={()=>{if(reborn.active){setReborn({active : false,reborn : itm.name})}}} key={index} src={`${itm.image}`}/>
            )
          })
        }
      </div>
      <div className="board-container">
            ...
            <div className="my-stats">
              <div className="user"><h3>{whoIAm ? myUserName : null}</h3></div>
              <div className="health"><h3>{myHp}</h3><span style={{width:`${myHp/200}%`}}></span></div>
              <div className="mana"><h3>{myMana}</h3><span style={{width:`${myMana/0.09}%`}}></span></div>
            </div>

            <div className="enemy-stats" style={reyAttacker ? {animation:"pulse 2s infinite"} : {animation:"none"}} 
            onClick={()=>{
              if(reyAttacker){
                setBaseDefender(true);
            }}}>
              <div className="user"><h3>{whoIsEnemy ? enemyUserName : null}</h3></div>
              <div className="health"><h3>{enemyHp}</h3><span style={{width:`${enemyHp/200}%`}}></span></div>
              <div className="mana"><h3>{enemyMana}</h3><span style={{width:`${enemyMana/0.09}%`}}></span></div>
            </div>

        <div className="board">
          <div className="enemy-container">

            <Enemytable whoIsJedi={whoIsJedi} locked={locked} setLocked={setLocked} bleed={bleed} setBleed={setBleed} setMindTrick={setMindTrick} mindTrick={mindTrick} setStealStats={setStealStats} stealStats={stealStats} myTurn={myTurn} enemyTurn={enemyTurn} cancelAttack={cancelAttack} setCancelAttack={setCancelAttack} enemyId={enemyId} cardTaken={cardTaken} setCardTaken={setCardTaken} setEnemyMana={setEnemyMana} enemyMana={enemyMana} setEnemyHp={setEnemyHp} enemyHp={enemyHp} setDefender={setDefender} attackPrepare={attackPrepare} setAttackPrepare={setAttackPrepare} client={client} setImg={setImg} side={`${mySide == 'light' ? 'dark' : 'light'}`}/>
          </div>
          <div className="my-container">
            <Mytable whoIsJedi={whoIsJedi} isMyFirstTurn={isMyFirstTurn} setLocked={setLocked} setTrashCopy={setTrashCopy} reborn={reborn} setReborn={setReborn} bleed={bleed} setBleed={setBleed} setMindTrick={setMindTrick} mindTrick={mindTrick} setStealStats={setStealStats} stealStats={stealStats} cancelAttack={cancelAttack} setCancelAttack={setCancelAttack} cardTaken={cardTaken} setCardTaken={setCardTaken} myTurn={myTurn} enemyTurn={enemyTurn} setMyTurn={setMyTurn} setEnemyTurn={setEnemyTurn} setBaseDefender={setBaseDefender} baseDefender={baseDefender} setReyAttacker={setReyAttacker} setEnemyHp={setEnemyHp} enemyHp={enemyHp} setMyMana={setMyMana} myMana={myMana} setMyHp={setMyHp} myHp={myHp} setDefender={setDefender} defender={defender} attackPrepare={attackPrepare} setAttackPrepare={setAttackPrepare} client={client} enemyId={enemyId} setImg={setImg} side={mySide} />
          </div>
        </div>
      </div>
    </div>
