...
export default function Mytable(props) {
    ...
    const changeCards = [
        function(data){ setM1(data)},
        function(data){ setM2(data)},
        function(data){ setM3(data)},
        function(data){ setM4(data)},
        function(data){ setM5(data)},
        function(data){ setM6(data)},
    ]
    ...
    const {whoIsJedi,isMyFirstTurn,setMyMana,myMana,setLocked,setTrashCopy,reborn,setReborn,bleed,setBleed,setMindTrick,mindTrick,stealStats,setStealStats,cardTaken,setCardTaken,defender,setDefender,myHp,setMyHp,enemyHp,setEnemyHp,setReyAttacker,baseDefender,setBaseDefender,myTurn,enemyTurn,setEnemyTurn,setMyTurn,cancelAttack,setCancelAttack} = props;
    ...
    useEffect(()=>{
        ...
        if(trash.length > 0){
            client.send(JSON.stringify({type: 'lastTrashCard',adress:props.enemyId,lastTrashCard:trash[trash.length-1]}));
        }else{
            client.send(JSON.stringify({type: 'lastTrashCard',adress:props.enemyId,lastTrashCard:''}));
        }
    },[trash])

    useEffect(()=>{
        client.send(JSON.stringify({type: 'enemy_force_cards',adress:props.enemyId,forceCards:cardWithForce}));
    },[cardWithForce])
    ...
    useEffect(()=>{
        cardWithForce.forEach((card,index)=>{
            switch(card.name){
                case 'darthvader':
                    if(!card.summoned){
                        client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:'',position:'all'}));
                        // setCardWithForce(cardWithForce.filter(el=>el.name !== 'darthvader'));
                        const copyArray = JSON.parse(JSON.stringify(cardWithForce));
                        const objIndex = copyArray.findIndex((obj => obj.name == 'darthvader'));
    ...
    useEffect(()=>{
        if(cardWithForce.find(x => x.name === 'obiwan')){
            const index = cardWithForce.map(object => object.name).indexOf('obiwan');
            const maxAttack = Math.max(...enemyForceCard.map(att => {
                if(att){
                    return att.attack
                }else return 0;
    ...
    useEffect(()=>{
        if(existKylo){
            ...
            client.send(JSON.stringify({type: 'hand_cards',adress:props.enemyId,card:handCards,existKylo:existKylo,numberOfCards:handCards.length}));
        }else {
            client.send(JSON.stringify({type: 'hand_cards',adress:props.enemyId,card:'',existKylo:existKylo,numberOfCards:handCards.length}));
    ...
    useEffect(()=>{
        if(cardWithForce.find(x => x.name === 'luke')){
            ...
            if(!(oldNumberOfDark.current == numberOfDark)){
                changeCards[index]({
                    ...getCards[index](),
                    attack : getCards[index]().attack + (numberOfDark-oldNumberOfDark.current)*100,
                });
        ...
        if(!isKyloOnField){
            client.send(JSON.stringify({type: 'kylo_ren',adress:props.enemyId,existKylo:false}));
        }
    ...
    useEffect(()=>{
        if(enemyHasVader){
            const copyArray = JSON.parse(JSON.stringify(cardWithForce));
            const indexOfYoda = cardWithForce.map(object => object.name).indexOf('yoda');
    ...
    useEffect(()=>{
        if(destroyAllLights){
            ...
            if(getCards[0]() && getCards[0]().name !== 'yoda' && getCards[0]().side === 'light'){
                copyTrash = [...copyTrash,cardWithForce[0]];
                setM1('');
                didMount.current.m1 = true;
            }
            if(getCards[1]() && getCards[1]().name !== 'yoda' && getCards[1]().side === 'light'){
                copyTrash = [...copyTrash,cardWithForce[1]];
                    setM2('');
                    didMount.current.m2 = true;
            }
            if(getCards[2]() && getCards[2]().name !== 'yoda' && getCards[2]().side === 'light'){
                copyTrash = [...copyTrash,cardWithForce[2]];
                    setM3('');
                    didMount.current.m3 = true;
            }
            if(getCards[3]() && getCards[3]().name !== 'yoda' && getCards[3]().side === 'light'){
                    copyTrash = [...copyTrash,cardWithForce[3]];
                    setM4('');
                    didMount.current.m4 = true;
            }
            if(getCards[4]() && getCards[4]().name !== 'yoda' && getCards[4]().side === 'light'){
                    copyTrash = [...copyTrash,cardWithForce[4]];
                    setM5('');
                    didMount.current.m5 = true;
            }
    ...
    function undoSeduceAndRedeemEffect(){
        if(cardTaken.newPosition !== ''){
            ...
            client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:{...getCards[cardTaken.newPosition](),seducedOrRedeemed : false},position:cardTaken.position.toString()}));
            setCardTaken({
                ...cardTaken,
                position : '',
                newPosition : '',
    ...
    useEffect(()=>{
        getCards.forEach((card,index)=>{
            let cursed = card();
            if(cursed && index !== 5){
                if(cursed.maulCurse.active && cursed.maulCurse.round == enemyTurn.round){
                    changeCards[index]('');
                }
            }
        })
    ...
    function letsFight(defender,attacker){
        if(!defender.fightCard.immune){
            const defence = defender.fightCard.isDef ? defender.fightCard.defence : defender.fightCard.attack;
            if(defence > attacker.fightCard.attack){
                if(!defender.fightCard.isDef){
                    changeCards[attacker.position-1]('');
    
                    if(attacker.fightCard.seducedOrRedeemed && attacker.fightCard.name !== 'darthsidious'){
                            client.send(JSON.stringify({type: 'addToTrash',adress:props.enemyId,addToTrash:attacker.fightCard}));
                    }
                    ...
                    switch(attacker.fightCard.name){
                        ...
                        if(attacker.fightCard.revived===0){
                            setTimeout(()=>{
                                changeCards[attacker.position-1]({...attacker.fightCard,revived:attacker.fightCard.revived+1,timesAttacked:attacker.fightCard.timesAttacked+1});
                            }
                                ,2000
                            )
                        }else{
                ...
                client.send(JSON.stringify({type: 'enemyHp',adress:props.enemyId,diference: newEnemyHp}));
                ...
                switch(defender.fightCard.name){
                    ...
                    case 'ahsoka':
                        if(attacker.fightCard.side ==='light'){
                            client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:{...defender.fightCard,attack : defender.fightCard.attack - 400},position:defender.position}))
                        }
                        break;
                }
            ...
            if(defence < attacker.fightCard.attack){
                if(defender.fightCard.isDef){
                    ...
                    client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:'',position:defender.position,diference:newEnemyHp}));
                ...
                switch(defender.fightCard.name){
                    case 'darthsidious':
                        if(defender.fightCard.revived===0){
                            ...
                            setTimeout(()=>{
                                client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:{...defender.fightCard,revived:defender.fightCard.revived+1,attack:defender.fightCard.attack + variable,defence:defender.fightCard.defence + variable},position:defender.position}))
                            }
                                ,2000
                            )
                        }
                    ...
                    case 'maul':
                        changeCards[attacker.position-1]({
                            ...attacker.fightCard,maulCurse : {active : true,round : myTurn.round + 1}
                        })
                        break;
            ...
            switch(attacker.fightCard.name){
                case 'quigon':
                    ...
                    const newAttacker = {
                        ...
                        fightCard : {
                            ...attacker.fightCard,
                            attack : chanceCube === 1 ? attacker.fightCard.attack + 700 : attacker.fightCard.attack - 700 ,
                            defence : chanceCube === 1 ? attacker.fightCard.defence + 700 : attacker.fightCard.defence - 700 ,
                        }
                    }
                    setTimeout(function(){letsFight(defender,newAttacker)},2000);
                break;
                ...
                case 'ahsoka':
                    ...
                    if(defender.fightCard.side ==='dark'){
                        ...
                        client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,card:{...defender.fightCard,attack : defender.fightCard.attack - 500,defence : defender.fightCard.defence - 500},position:defender.position}))
                        setTimeout(function(){letsFight(newDefender,attacker)},2000);
                    }
    ...
    function forceAdd(m,position){
        ...
        switch(m.name){
            ...
            case 'kyloren':
                    client.send(JSON.stringify({type: 'kylo_ren',adress:props.enemyId,existKylo:true}));

                break;
        }
        var newArray = JSON.parse(JSON.stringify(cardWithForce));
                newArray[position] = m
                if(m.name === 'grievous' && !cardWithForce.find((el)=> el.name == 'grievous')){
                    let freeSpace =0;
                    let indexOfDroids = [];
        ...
        switch(name){
            ...
            case 'special_order66':
                ...
                if(isDarkInMyTeam || isWinduOnTable){
                    ...
                    setTimeout(()=>{
                        if(cardsDestroyed){
                            client.send(JSON.stringify({type: 'talk_to_attacker',adress:props.enemyId,position:'all_light'}));
                            changeCards[position]('');
                        }
                    },1500);
                }else {
                ...
                case 'special_seduce':
                    if((isDarkInMyTeam && isEnemyOnTable) || (isWinduOnTable && isEnemyOnTable)){
                        setTimeout(()=>{
                            setCardTaken({
                                ...cardTaken,
                                active : true,
                ...
                case 'special_reedem':
                    if((isLightInMyTeam && isEnemyOnTable) || (isWinduOnTable && isEnemyOnTable)){
                        setTimeout(()=>{
                            setCardTaken({
                                ...cardTaken,
                                active : true,
                ...
                case 'special_traiders':
                        setTimeout(()=>{
                            client.send(JSON.stringify({type: 'no_force_cards',adress:props.enemyId}));
                            changeCards[position]('');
                        },1500);
                    break;
                ...
                case 'special_saberthrow':
                        setTimeout(()=>{
                            client.send(JSON.stringify({type: 'no_summon',adress:props.enemyId}));
                            changeCards[position]('');
                        },1500);
                    break;
                ...
                case 'special_forcelightning':
                    setTimeout(()=>{
                        ...
                        client.send(JSON.stringify({type: 'myHp',adress:props.enemyId,diference:1000,sign:'minus'}));
                        changeCards[position]('');
                    },1500);
                break;
                ...
                case 'special_forcehealing':
                    setTimeout(()=>{
                        ...
                        client.send(JSON.stringify({type: 'myHp',adress:props.enemyId,diference:1000,sign:'plus'}));
                        changeCards[position]('');
                    },1500);
                break;
                ...
                case 'special_binarysunset':
                    if(handCards.length<=8 && deck.length>=2){
                        setTimeout(()=>{
                            ...
                            const newDeck = deck.filter((element,index)=> index !== deck.length-1);
                            const newHandCards = handCards.concat(lastCard1);

                            const lastCard2 = deck[deck.length-2];

                            setDeck(newDeck.filter((element,index)=> index !== deck.length-1));
    ...
    useEffect(()=>{
        if(!didMount.current.m1){
            if(!m1 && cardWithForce[0] && cardWithForce[0].revived !== 0 && cardWithForce[0].name !== 'droid'){
                ...
                forceAdd(m1,0);
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m1,position:'1'}));
        }else{
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m1,position:'1'}));
    ...
    useEffect(()=>{
        if(!didMount.current.m2){
            if(!m2 && cardWithForce[1] && cardWithForce[1].revived !== 0 && cardWithForce[1].name !== 'droid'){
                ...
                forceAdd(m2,1);
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m2,position:'2'}));
        }else{
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m2,position:'2'}));
    ...
    useEffect(()=>{
        if(!didMount.current.m3){
            if(!m3 && cardWithForce[2] && cardWithForce[2].revived !== 0 && cardWithForce[2].name !== 'droid'){
                ...
                forceAdd(m3,2);
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m3,position:'3'}));
        }else{
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m3,position:'3'}));
    ...
    useEffect(()=>{
        ...
        if(!didMount.current.m4){
            if(!m4 && cardWithForce[3] && cardWithForce[3].revived !== 0 && cardWithForce[3].name !== 'droid'){
                ...
                forceAdd(m4,3);
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m4,position:'4'}));
        }else{
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m4,position:'4'}));
    ...
    useEffect(()=>{
        if(!didMount.current.m5){
            if(!m5 && cardWithForce[4] && cardWithForce[4].revived !== 0 && cardWithForce[4].name !== 'droid'){
                ...
                forceAdd(m5,4);
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m5,position:'5'}));
        }else{
            client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m5,position:'5'}));
    ...
    useEffect(()=>{
        client.send(JSON.stringify({type: 'talk_to',adress:props.enemyId,card:m6,position:'6'}));
  ...
  return (
    <div className="my-card-container" style={!myTurn.turn ? {pointerEvents:"none"}: {pointerEvents:"all"}}>

        <div className="my-hand-container">
            
            <HandCards myMana={myMana} setMyMana={setMyMana} noSummon={noSummon} noForceCards={noForceCards} cardToDeploy={cardToDeploy} specialToDeploy={specialToDeploy} myFirstCards={handCards} setImg={props.setImg} setSpecialToDeploy={setSpecialToDeploy} setCardToDeploy={setCardToDeploy}/>
        ...
        <div className="my-card-table">
            ...
            onClick={()=>{
                if(specialToDeploy){
                        ...
                        setHandCards(handCards.filter(element => element.id !== specialToDeploy.id));
            ...
            name='m6' id={`${!m6 && specialToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m6 ? 'full' : 'empty'}`}>
                <img
                className='special-card'
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
            ...
            onClick={()=>{
                if(cardToDeploy && !m1){
                    ...
                    setHandCards(handCards.filter(element => element.id !== cardToDeploy.id))
                ...
                setTimeout(function(){
                    if(single){
                        ...
                        }else if((m1 && !m1.isDef && m1.timesAttacked == 0)||(m1 && !m1.isDef && m1.timesAttacked == 1 && m1.name === 'dooku')){
                            if(!isMyFirstTurn.current){
                                if(cancelAttack.position != 1 && !m1.wasDef){
                                    props.setAttackPrepare(true);
                                    setAttacker({position:'1',fightCard:m1});
                                    client.send(JSON.stringify({type: 'will_attack',adress:props.enemyId,position:'1'}));
                                }
                            }
                        }
                    }
            ...
            name='m1' id={`${!m1 && cardToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m1 ? 'full' : 'empty'}`}>
                <img
                className='fight-card'
                style={Object.assign({},m1.isDef ? {transform:"rotate(90deg)"} : {transform:"rotate(0deg)"},attacker.position === '1' || willAttack === '1' || mindTricks.attacker === '1' || mindTricks.defender === '1' ? {animation: "shake 0.5s",animationIterationCount: "infinite"} : {animation: "none",animationIterationCount: "unset"})}
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
                ...
                {m1 ? m1.maulCurse.active ? <img src={darthmaul} className='sword'/> : '' : ''}
                {sithBetrayal.destroying && m1? <img src={fire} className='sword'/> : ''}
                {sithBetrayal.boosting && m1 ? <img src={boost} className='sword'/> : ''}
                {m1 && stealStats.stolen ? <img src={pulse} className='sword'/> : ''}
                {m1 && (forcePush.boosting || kyberCrystal.boosting || forcePull.boosting) ? <img src={pulse} className='sword'/> : ''}
                {m1.side == 'light' && forceShield.active ? <img src={pulse} className='sword'/> : ''}
                <div id='a' style={attacker.position === '1' || willAttack === '1' || mindTricks.attacker === '1' || mindTricks.defender === '1' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='b' style={attacker.position === '1' || willAttack === '1' || mindTricks.attacker === '1' || mindTricks.defender === '1' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='c' style={attacker.position === '1' || willAttack === '1' || mindTricks.attacker === '1' || mindTricks.defender === '1' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='d' style={attacker.position === '1' || willAttack === '1' || mindTricks.attacker === '1' || mindTricks.defender === '1' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
            </div>
            ...
            onClick={()=>{
                ...
                setTimeout(function(){
                    if(single){
                        ...
                        }else if((m2 && !m2.isDef && m2.timesAttacked == 0)||(m2 && !m2.isDef && m2.timesAttacked == 1 && m2.name === 'dooku')){
                            if(!isMyFirstTurn.current){
                                if(cancelAttack.position != 2 && !m2.wasDef){
                                    props.setAttackPrepare(true);
                                    setAttacker({position:'2',fightCard:m2});
                                    client.send(JSON.stringify({type: 'will_attack',adress:props.enemyId,position:'2'}));
                                }
                            }
                        }
                    }
            ...
            name='m2' id={`${!m2 && cardToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m2 ? 'full' : 'empty'}`}>
                <img
                className='fight-card'
                style={Object.assign({},m2.isDef ? {transform:"rotate(90deg)"} : {transform:"rotate(0deg)"},attacker.position === '2' || willAttack === '2' || mindTricks.attacker === '2' || mindTricks.defender === '2' ? {animation: "shake 0.5s",animationIterationCount: "infinite"} : {animation: "none",animationIterationCount: "unset"})}
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
                ...
                {m2 ? m2.maulCurse.active ? <img src={darthmaul} className='sword'/> : '' : ''}
                {sithBetrayal.destroying && m2? <img src={fire} className='sword'/> : ''}
                {sithBetrayal.boosting && m2 ? <img src={boost} className='sword'/> : ''}
                {m2 && stealStats.stolen ? <img src={pulse} className='sword'/> : ''}
                {m2 && (forcePush.boosting || kyberCrystal.boosting || forcePull.boosting) ? <img src={pulse} className='sword'/> : ''}
                {m2.side == 'light' && forceShield.active ? <img src={pulse} className='sword'/> : ''}
                <div id='a' style={attacker.position === '2' || willAttack === '2' || mindTricks.attacker === '2' || mindTricks.defender === '2' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='b' style={attacker.position === '2' || willAttack === '2' || mindTricks.attacker === '2' || mindTricks.defender === '2' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='c' style={attacker.position === '2' || willAttack === '2' || mindTricks.attacker === '2' || mindTricks.defender === '2' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='d' style={attacker.position === '2' || willAttack === '2' || mindTricks.attacker === '2' || mindTricks.defender === '2' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
            </div>
            ...
            onClick={()=>{
                ...
                setTimeout(function(){
                    if(single){
                        ...
                        }else if((m3 && !m3.isDef && m3.timesAttacked == 0)||(m3 && !m3.isDef && m3.timesAttacked == 1 && m3.name === 'dooku')){
                            if(!isMyFirstTurn.current){
                                if(cancelAttack.position != 3 && !m3.wasDef){
                                    props.setAttackPrepare(true);
                                    setAttacker({position:'3',fightCard:m3});
                                    client.send(JSON.stringify({type: 'will_attack',adress:props.enemyId,position:'3'}));
                                }
                            }
                        }
                    }
            ...
            name='m3' id={`${!m3 && cardToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m3 ? 'full' : 'empty'}`}>
                <img
                className='fight-card'
                style={Object.assign({},m3.isDef ? {transform:"rotate(90deg)"} : {transform:"rotate(0deg)"},attacker.position === '3' || willAttack === '3' || mindTricks.attacker === '3' || mindTricks.defender === '3' ? {animation: "shake 0.5s",animationIterationCount: "infinite"} : {animation: "none",animationIterationCount: "unset"})}
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
                ...
                {m3 ? m3.maulCurse.active ? <img src={darthmaul} className='sword'/> : '' : ''}
                {sithBetrayal.destroying && m3? <img src={fire} className='sword'/> : ''}
                {sithBetrayal.boosting && m3 ? <img src={boost} className='sword'/> : ''}
                {m3 && stealStats.stolen ? <img src={pulse} className='sword'/> : ''}
                {m3 && (forcePush.boosting || kyberCrystal.boosting || forcePull.boosting) ? <img src={pulse} className='sword'/> : ''}
                {m3.side == 'light' && forceShield.active ? <img src={pulse} className='sword'/> : ''}
                <div id='a' style={attacker.position === '3' || willAttack === '3' || mindTricks.attacker === '3' || mindTricks.defender === '3' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='b' style={attacker.position === '3' || willAttack === '3' || mindTricks.attacker === '3' || mindTricks.defender === '3' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='c' style={attacker.position === '3' || willAttack === '3' || mindTricks.attacker === '3' || mindTricks.defender === '3' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='d' style={attacker.position === '3' || willAttack === '3' || mindTricks.attacker === '3' || mindTricks.defender === '3' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
            </div>
            ...
            onClick={()=>{
                ...
                setTimeout(function(){
                    if(single){
                        ...
                        }else if((m4 && !m4.isDef && m4.timesAttacked == 0)||(m4 && !m4.isDef && m4.timesAttacked == 1 && m4.name === 'dooku')){
                            if(!isMyFirstTurn.current){
                                if(cancelAttack.position != 4 && !m4.wasDef){
                                    props.setAttackPrepare(true);
                                    setAttacker({position:'4',fightCard:m4});
                                    client.send(JSON.stringify({type: 'will_attack',adress:props.enemyId,position:'4'}));
                                }
                            }
                        }
                    }
            ...
            name='m4' id={`${!m4 && cardToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m4 ? 'full' : 'empty'}`}>
                <img
                className='fight-card'
                style={Object.assign({},m4.isDef ? {transform:"rotate(90deg)"} : {transform:"rotate(0deg)"},attacker.position === '4' || willAttack === '4' || mindTricks.attacker === '4' || mindTricks.defender === '4' ? {animation: "shake 0.5s",animationIterationCount: "infinite"} : {animation: "none",animationIterationCount: "unset"})}
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
                ...
                {m4 ? m4.maulCurse.active ? <img src={darthmaul} className='sword'/> : '' : ''}
                {sithBetrayal.destroying && m4? <img src={fire} className='sword'/> : ''}
                {sithBetrayal.boosting && m4 ? <img src={boost} className='sword'/> : ''}
                {m4 && stealStats.stolen ? <img src={pulse} className='sword'/> : ''}
                {m4 && (forcePush.boosting || kyberCrystal.boosting || forcePull.boosting) ? <img src={pulse} className='sword'/> : ''}
                {m4.side == 'light' && forceShield.active ? <img src={pulse} className='sword'/> : ''}
                <div id='a' style={attacker.position === '4' || willAttack === '4' || mindTricks.attacker === '4' || mindTricks.defender === '4' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='b' style={attacker.position === '4' || willAttack === '4' || mindTricks.attacker === '4' || mindTricks.defender === '4' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='c' style={attacker.position === '4' || willAttack === '4' || mindTricks.attacker === '4' || mindTricks.defender === '4' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='d' style={attacker.position === '4' || willAttack === '4' || mindTricks.attacker === '4' || mindTricks.defender === '4' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
            </div>
            ...
            onClick={()=>{
                ...
                setTimeout(function(){
                    if(single){
                        ...
                        }else if((m5 && !m5.isDef && m5.timesAttacked == 0)||(m5 && !m5.isDef && m5.timesAttacked == 1 && m5.name === 'dooku')){
                            if(!isMyFirstTurn.current){
                                if(cancelAttack.position != 5 && !m5.wasDef){
                                    props.setAttackPrepare(true);
                                    setAttacker({position:'5',fightCard:m5});
                                    client.send(JSON.stringify({type: 'will_attack',adress:props.enemyId,position:'5'}));
                                }
                            }
                        }
                    }
            ...
            name='m5' id={`${!m5 && cardToDeploy ? 'hovered' : 'unhovered'}`} className={`card ${m5 ? 'full' : 'empty'}`}>
                <img
                className='fight-card'
                style={Object.assign({},m5.isDef ? {transform:"rotate(90deg)"} : {transform:"rotate(0deg)"},attacker.position === '5' || willAttack === '5' || mindTricks.attacker === '5' || mindTricks.defender === '5' ? {animation: "shake 0.5s",animationIterationCount: "infinite"} : {animation: "none",animationIterationCount: "unset"})}
                onMouseOver={(e)=>{props.setImg({img : e.target.src, description : e.target.alt})}}
                onMouseOut={()=>{props.setImg({img : '',description : ''})}}
                ...
                {m5 ? m5.maulCurse.active ? <img src={darthmaul} className='sword'/> : '' : ''}
                {sithBetrayal.destroying && m5? <img src={fire} className='sword'/> : ''}
                {sithBetrayal.boosting && m5 ? <img src={boost} className='sword'/> : ''}
                {m5 && stealStats.stolen ? <img src={pulse} className='sword'/> : ''}
                {m5 && (forcePush.boosting || kyberCrystal.boosting || forcePull.boosting) ? <img src={pulse} className='sword'/> : ''}
                {m5.side == 'light' && forceShield.active ? <img src={pulse} className='sword'/> : ''}
                <div id='a' style={attacker.position === '5' || willAttack === '5' || mindTricks.attacker === '5' || mindTricks.defender === '5' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='b' style={attacker.position === '5' || willAttack === '5' || mindTricks.attacker === '5' || mindTricks.defender === '5' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='c' style={attacker.position === '5' || willAttack === '5' || mindTricks.attacker === '5' || mindTricks.defender === '5' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
                <div id='d' style={attacker.position === '5' || willAttack === '5' || mindTricks.attacker === '5' || mindTricks.defender === '5' ? {visibility: "visible"} : {visibility: "hidden"}} className="pulse"></div>
            </div>
        ...
        <div className="extra-space">
            ...
            <span className='cards-counter'>{deck.length}</span>
                ...
                id='deck' draggable='false' src={`${whoIsJedi.current ? 'https://i.imgur.com/91Szz36.jpg' : 'https://i.imgur.com/J8aBfLG.png' }`} alt="" />
            </div>
            <div onClick={(event)=>{event.nativeEvent.path[6].children[4].style.display = 'block';console.log(event)}} className="trash">
                    
                    <img src={`${trash.length>0 ? trash[trash.length-1].image : ''}`}/>
        ...
        className='btn'><a style={whoIsJedi.current ? {background: '#5082f0'} : {background: '#f44336'}} href='#'>Cancel <br/>attack</a></div>
        :
        cardToDeploy || specialToDeploy 
        ?
        <div 
        onClick={()=>{setCardToDeploy('');setSpecialToDeploy('')}} className='btn'><a style={whoIsJedi.current ? {background: '#5082f0'} : {background: '#f44336'}} href='#'>Cancel <br/>selection</a></div>
        :
        <div
        onClick={()=>{
            if(myTurn.turn){
                ...
                client.send(JSON.stringify({type: 'your_turn',adress: props.enemyId,turn:{turn:true,round:enemyTurn.round+1}}));
                client.send(JSON.stringify({type: 'my_turn',adress: props.enemyId,turn:{turn:false,round:myTurn.round}}));
        ...
        className="btn" ><a style={whoIsJedi.current ? {background: '#5082f0'} : {background: '#f44336'}} href='#'>End turn</a></div>
        :
        ''
        }
