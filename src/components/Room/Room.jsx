...
function Room({client}) {
  ...
  const [createJoinName, setJoinUserName] = useState('');
  const [createJoinNumber, setJoinNumber] = useState('');
  ...
  useEffect(()=>{
    ...
    client.onmessage = (message) => {
      const dataFromServer = JSON.parse(message.data);
          ...
          }else if(dataFromServer.type==='unique_id'){
            setUniqueId(dataFromServer.key)
          }
  ...
  function existRoom(number){
    let doesExist = false;
    for(let elem in data){
      if(data[elem].room == number){
        doesExist = true;
      }
    }
    return doesExist
  ...
  function createRoom(e) {
    e.preventDefault()
    if(data){
      if(!existRoom(createRoomNumber)){
        client.send(JSON.stringify({
          type: 'data_create',
          state : {
          'room' : createRoomNumber,
          'players' : {
            'host' : {
        ...
        navigate(`/loading/${createRoomNumber}/${uniqueId}`, { replace: true })
      } else return console.log('camera deja exista')
    }else{
      client.send(JSON.stringify({
        type: 'data_create',
        state : {
  ...
  function joinRoom(e){
    e.preventDefault();
    if(data){
      if(existRoom(createJoinNumber) && data[data.findIndex(obj=>obj.room === createJoinNumber)].players.guest.id == ''){
        const copyData = JSON.parse(JSON.stringify(data))
        const updatedArray = copyData.map(p =>
          p.room === createJoinNumber
            ? { ...p, players: {...p.players,guest:{username:createJoinName,id:uniqueId}} }
            : p
        );

        client.send(JSON.stringify({type: 'data_update',state :updatedArray}));
        navigate(`/loading/${createJoinNumber}/${uniqueId}`, { replace: true })
      } else console.log('room doesnt exist or is full')
    }else{
    ...
    <div className="twinkle"></div>
      {
...

        {
          ...
          <div className="game-mode-container">
            ...
            <div onClick={()=>{setGameMode('duel_of_fates')}} className="box">

              <div className="imgBx">
                  <img src="http://samuel-garcia.site/img/the-force.png" alt="STAR-WARS-THE-FORCE"/>
              </div>
              <div className="content">
                  <h2>STAR WARS<br/><span>the duel of fates</span></h2>
              </div>
            ...
            <div className="box">
                ...
                <div className="content">
                    <h2>STAR WARS<br/><span>Coming soon...</span></h2>
                </div>
            ...
            type="text"
            placeholder='name...'
            value={createJoinName}
            onChange={e => setJoinUserName(e.target.value)}
            />
            <input 
            type="text"
            placeholder='room...'
            value={createJoinNumber}
          ...
          :
          <div className="game-mode-container">
            <form className='create' onSubmit={createRoom} >
              ...
              <span></span>
              Create</button>
              <button type='button' onClick={()=>{setGameMode('')}}>
              <span></span>
              <span></span>
              <span></span>
              <span></span>
        ...
        <div onClick={()=>{setMenu(true)}} className="container">
          <div className="intro">
      ...
      <div className="prologue">
        <div className="text">
          <p>For all those who have created this universe.</p>
          <p>For all the crews of all the movies, even those made in CGI.</p>
          <p>For all the Republic and the Rebellion's planets.</p>
          <p>For all the Death Stars that have been destroyed.</p>
          <p>For all the ladies we lost.</p>
          <p>For all the Jedi and for all the Sith.</p>
          <p>For all those who love Star Wars, and for all those will love it....</p>
        </div>
    </div>
